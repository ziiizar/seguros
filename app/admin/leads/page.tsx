...
const page = async ({
  ...
  const enrolledLeads =
    allLeads?.filter((lead) => lead.status === "ENROLLED") || [];
  ...
  const activeClientsProgress = Math.min(
    (activeClients / activeClientsTarget) * 100,
    100
  );

  return (
    <main className="flex flex-col bg-zinc-800 rounded-xl p-3 h-full overflow-y-hidden">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        ...
        <Card className="bg-zinc-900 border-zinc-800 transition-all hover:border-zinc-700">
          ...
          <CardContent>
            <div className="space-y-3">
              <CardDescription className="text-zinc-400">
                Target: {activeClientsTarget} clients
              </CardDescription>
              <div className="h-2 w-full bg-zinc-800 rounded-full overflow-hidden">
                <div
              ...
              <div className="flex items-center justify-between">
                ...
                <p className="text-sm font-medium text-blue-500">
                  {activeClients} / {activeClientsTarget}
                </p>
              </div>
            </div>
          </CardContent>
